...
int n;
  for (n = 0; ('0' <= string[*offset]) && ('9' >= string[*offset]); *offset = *offset + 1) {
    n = (n * 10) + string[*offset] - '0';
  }
  return n;
}
...
int passwordValid;
  ...
  for (passwordValid = offset = 0; '*' != passwordString[offset];) {
    min = getNumber(passwordString
                  , &offset);
    ++offset;
    max = getNumber(passwordString
                  , &offset);
    letter = passwordString[++offset];
    offset += 3;
    for (letterCount = 0; ' ' != passwordString[offset]; ++offset) {
      if ((letter + 0) == passwordString[offset]) {
        ++letterCount;
      }
    }
    if (((min + 0) <= letterCount) && ((max + 0) >= letterCount)) {
      ++passwordValid;
    }
    for (; ' ' == passwordString[offset]; ++offset) {  }
  }


	puts("Have First Answer"); /* prints !!!Hello World!!! */
  ...
  for (passwordValid = offset = 0; '*' != passwordString[offset];) {
    ...
    letter = passwordString[++offset];
    offset += 2;
    if ((((letter + 0) == passwordString[offset + min]) && ((letter + 0) != passwordString[offset + max])) ||
        (((letter + 0) != passwordString[offset + min]) && ((letter + 0) == passwordString[offset + max]))) {
      ++passwordValid;
    }
    for (++offset; ' ' != passwordString[offset]; ++offset) {  }
    for (        ; ' ' == passwordString[offset]; ++offset) {  }
  }
